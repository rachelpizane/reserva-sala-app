...
describe(NotFound.name, () => {
  ...
  it('deve exibir o título de página não encontrada e o botão de voltar', () => {
    ...
    expect(screen.getByRole('button', { name: /voltar para página inicial/i })).toBeInTheDocument()
  });

  it("deve navegar para página inicial ao clicar no botão", () => {
    render(<NotFound />);

    const button: HTMLButtonElement = screen.getByRole('button', { name: /voltar para página inicial/i})
